...
//Insertion
void insertionSort(Ordenador* lista){
    int n = lista->numElementos;
    int i, j;
    Personagens* key;
    for (i = 1; i < n; i++) {
        key = lista->ordem[i];
        j = i - 1;
        while (j >= 0 && strcmp(lista->ordem[j]->nome->string, key->nome->string) > 0 && lista->ordem[j]->altura == key->altura) {
