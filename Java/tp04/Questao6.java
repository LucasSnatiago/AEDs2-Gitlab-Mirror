...
public class Questao6 {
    public static void main(String[] args){
        ...
        while(!ehFim(entrada)) {
            ...
            if(tabela.pesquisar(entrada)) MyIO.println("SIM");
            else System.out.println("N" + (char)195 + "O");
...
class TabelaHash {
    protected int tamTabela;
    protected int tamReserva;
    private Personagem[] tabela;

    // Construtor de Tabela Hash
    public TabelaHash() {
        this.tamTabela = 21;
        this.tamReserva = 9;

        this.tabela = _criarTabelaLimpa();
    }

    // Criar uma tabela limpa de personagens
    private Personagem[] _criarTabelaLimpa() {
        int tamTabela = this.tamTabela + this.tamReserva;
        Personagem[] tabela = new Personagem[tamTabela];
    ...
    // Metodo de inserir na tabela hash
    public void inserir(Personagem personagem) {
        int posTabela = personagem.getAltura() % this.tamTabela;
        int rePosTabela = (personagem.getAltura() + 1) % this.tamTabela;
    
        if(this.tabela[posTabela] == null)
            this.tabela[posTabela] = personagem;
            
        else if(this.tabela[rePosTabela] == null)
            this.tabela[rePosTabela] = personagem;

        else {
            for(int i = this.tamTabela; i < (this.tamReserva + this.tamTabela); i++) {
                if(this.tabela[i] == null){
                    this.tabela[i] = personagem;
                    i = this.tamReserva + this.tamTabela;
    ...
    public boolean pesquisar(String nome) {
        ...
        for(int i = 0; i < this.tamReserva + this.tamTabela; i++) {
            if(this.tabela[i] != null && this.tabela[i].getNome().compareTo(nome) == 0) {
                existeTabela = true;
                i = this.tamReserva + this.tamTabela;
            }
        }
