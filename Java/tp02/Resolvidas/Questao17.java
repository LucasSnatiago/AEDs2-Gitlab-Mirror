...
public class Questao17 {
    public static void main(String[] args){
        ...
        log.cronometroStart();
        ps.ordenar(log, 0, ps.numElementos-1);
        log.cronometroStop();

        log.criarLog("matrícula_mergesort.txt");
        ps.consertarNomes();
        ps.escreverLista();
...
class AlgoritmoOrdenacao extends Ordenador {
    ...
    public void ordenar(Arquivo log, int indiceInicio, int indiceFim) {
        // Condicional que verifica a validade dos parâmetros passados.
		if (this.lista != null && indiceInicio < indiceFim && indiceInicio >= 0 &&
        indiceFim < this.lista.length && this.lista.length != 0) {
           int meio = ((indiceFim + indiceInicio) / 2);

           ordenar(log, indiceInicio, meio);
           ordenar(log, meio + 1, indiceFim);

           merge(log, indiceInicio, meio, indiceFim);
       }
    }

    public void merge(Arquivo log, int indiceInicio, int meio, int indiceFim) {

		Personagem[] auxiliar = new Personagem[this.numElementos];
		//Copiando o trecho da lista que vai ser ordenada
		for (int i = indiceInicio; i <= indiceFim; i++) {
            auxiliar[i] = this.lista[i];
            log.numMovimentacoes++;
		}
		...
		while (i <= meio && j <= indiceFim) {
			if (auxiliar[i].getHomeworld().compareTo(auxiliar[j].getHomeworld()) < 0) {
                this.lista[k] = auxiliar[i];
                log.numMovimentacoes++;
				i++;
			} else {
                this.lista[k] = auxiliar[j];
                log.numMovimentacoes++;
				j++;
			}
            k++;
    ...
    public void consertarNomes(){
        ...
        for (int i = 1; i < this.numElementos; i++) {
            ...
            while ((j >= 0) && verificarOrdemAlfabetica(this.lista[j].getNome(), tmp.getNome()) == 2 && this.lista[j].getHomeworld().equals(tmp.getHomeworld())) {
