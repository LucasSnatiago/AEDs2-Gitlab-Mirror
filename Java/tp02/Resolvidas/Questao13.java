...
class AlgoritmoOrdenacao extends Ordenador {
    ...
    private void swapHeap(int a, int b, Arquivo log){
        Personagem tmp = this.lista[a];
        this.lista[a] = this.lista[b];
        this.lista[b] = tmp;
        log.numMovimentacoes += 2;
    }

    public void constroi(int tamHeap, Arquivo log){
        for(int i = tamHeap; i > 1 && this.lista[i].getAltura() > this.lista[i/2].getAltura(); i /= 2){
            swapHeap(i, i/2, log);
            log.numMovimentacoes++;
            log.numComparacoes++;
        }
    ...
    public void reconstroi(int tamHeap, Arquivo log){
        int i = 1, filho;
        while(i <= (tamHeap/2)){
  
           if (this.lista[2*i].getAltura() > this.lista[2*i+1].getAltura() || 2*i == tamHeap){
                filho = 2*i;
                log.numComparacoes+=2;
           } else {
                filho = 2*i + 1;
           }
  
           if(this.lista[i].getAltura() < this.lista[filho].getAltura()){
                swapHeap(i, filho, log);
                i = filho;
           }else{
                i = tamHeap;
           }
      ...
      public void ordenar(Arquivo log) {
        Personagem[] tmp = new Personagem[this.numElementos+1];
        for(int i = 0; i < this.numElementos; i++){
           tmp[i+1] = this.lista[i];
           log.numMovimentacoes++;
        }
        this.lista = tmp;
        ...
        for(int tamHeap = 2; tamHeap <= this.numElementos; tamHeap++){
           constroi(tamHeap, log);
        }
  
        //Ordenando
        int tamHeap = this.numElementos;
        while(tamHeap > 1){
            swapHeap(1, tamHeap--, log);
            reconstroi(tamHeap, log);
        }
  
        //Alterar o vetor para voltar a posicao zero
        tmp = this.lista;
        this.lista = new Personagem[this.numElementos];
        for(int i = 0; i < this.numElementos; i++){
           this.lista[i] = tmp[i+1];
           log.numMovimentacoes++;
        }
    }
    ...
    public void consertarNomes(){
        ...
        for (int i = 1; i < this.numElementos; i++) {
            ...
            while ((j >= 0) && verificarOrdemAlfabetica(this.lista[j].getNome(), tmp.getNome()) == 2 && (this.lista[j].getAltura() == tmp.getAltura())) {
