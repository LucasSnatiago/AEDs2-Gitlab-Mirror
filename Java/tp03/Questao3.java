...
class Fila extends Celula {
    ...
    //Orquestrar execucoes
    public void ordenador(String[] texto) {
    ...
    private void _quicksort(int esq, int dir) {
        ...
        Celula pivo = this.inicio;
        
        for(int tmp = 0; tmp < i; tmp++, celI = celI.prox);
        for(int tmp = 0; tmp < j; tmp++, celJ = celJ.prox);
        for(int tmp = 0; tmp < (i+j)/2; tmp++, pivo = pivo.prox);
        
        while (i <= j) {
            while (celI.personagem.getCorDoCabelo().compareTo(pivo.personagem.getCorDoCabelo()) < 0){ 
                celI = celI.prox;
                i++;
            }
            while (celJ.personagem.getCorDoCabelo().compareTo(pivo.personagem.getCorDoCabelo()) > 0){
                celJ = celJ.ant;
                j--;
            }
